    ...
    <div class="container-fluid" >
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <ol class="breadcrumb">
                    <li>
                        <i class="fa fa-dashboard"></i>  <a href="/bigticket/admin.php?m=admin&c=menu&a=index">菜单管理</a>
                    </li>
                    <li class="active">
                        <i class="fa fa-table"></i><?php echo ($nav); ?> 菜单列表
                    </li>
                </ol>
            </div>
        </div>
            ...
            <form action="/bigticket/admin.php" method="get">
                <div class="input-group">
                    ...
                    <select class="form-control" name="type">
                        <option value='2'  <?php if($type): ?>selected="selected"<?php endif; ?>>综合</option>
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-hover singcms-table">
                        ...
                        <?php if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>  
                                <td><input type="text" size="4" name="listorder[<?php echo ($menu["menu_id"]); ?>]" value="<?php echo ($menu["listorder"]); ?>" /></td>                       
                                <td><?php echo ($menu["menu_id"]); ?></td>
                                <td><?php echo ($menu["name"]); ?></td>
                                <td><?php echo ($menu["m"]); ?></td>
                                <td><?php echo (getMenuType($menu["type"])); ?></td>
                                <td><if><span attr-status="<?php if($menu['status'] == 1): ?>0<?php else: ?>1<?php endif; ?>" attr-id="<?php echo ($menu["menu_id"]); ?>" class="sing_cursor singcms-on-off" id="singcms-on-off"><?php echo (status($menu["status"])); ?></span></td>
                                <td>
                                <span class="glyphicon glyphicon-edit" aria-hidden="true" id="singcms-edit" attr-id="<?php echo ($menu["menu_id"]); ?>" style="cursor:pointer"></span>    
...
<script>
    ...
    var SCOPE = {
        'add_url' : '/bigticket/admin.php?c=menu&a=add',
        'edit_url' : '/bigticket/admin.php?c=menu&a=edit',
        'set_status_url' : '/bigticket/admin.php?c=menu&a=setStatus',
        'listorder_url' : '/bigticket/admin.php?c=menu&a=listorder',
