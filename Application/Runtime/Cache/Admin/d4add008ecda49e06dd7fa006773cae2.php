    ...
    <div class="container-fluid" >
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <ol class="breadcrumb">
            <li>
              <i class="fa fa-dashboard"></i>  <a href="/bigticket/admin.php?c=ticket">订单管理</a>
            </li>
            <li class="active">
      ...
      <!-- /.row -->
      <div >
        <button  id="button-add" type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span>购票 </button>
      </div>
      <br/>
      <div class="row">
        <form action="/bigticket/admin.php" method="get">
          ...
          <div class="col-md-3">
            <div class="input-group">
              ...
              <input class="form-control" name="buyer" type="text" value="<?php echo ($buyer); ?>" placeholder="购买者" />
          ...
          <div class="">
            <form id="singcms-listorder">
              <table class="table table-bordered table-hover singcms-table">
                ...
                <tbody>
                <?php if(is_array($ticket)): $i = 0; $__LIST__ = $ticket;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ticket): $mod = ($i % 2 );++$i;?><tr>
                    <td><input type="checkbox" name="pushcheck" value="<?php echo ($ticket["ticket_id"]); ?>"></td>
                    <td><input size=4 type='text'  name='listorder[<?php echo ($new["news_id"]); ?>]' value="<?php echo ($new["listorder"]); ?>"/></td><!--6.7-->
                    <td><?php echo ($ticket["ticket_id"]); ?></td>
                    <td><?php echo ($ticket["title"]); ?></td>
                    <td><?php echo (date("Y-m-d",$ticket["time"])); ?></td>
                    <td><?php echo (date("H:i",$ticket["time"])); ?></td>
                    <td><?php echo ($ticket["hall"]); ?></td>
                    <td><?php echo ($ticket["number"]); ?></td>
                    <td><?php echo ($ticket["totalprice"]); ?></td>
                    <td><?php echo ($ticket["buyer"]); ?></td>
                    <td><span  attr-status="<?php if($ticket['status'] == 1): ?>0<?php else: ?>1<?php endif; ?>" attr-id="<?php echo ($ticket["ticket_id"]); ?>" class="sing_cursor singcms-on-off" id="singcms-on-off" ><?php echo (status($ticket["status"])); ?></span></td>
...
<script>
  var SCOPE = {
    'jump_url' :'/bigticket/admin.php?c=ticket',
    'edit_url' : '/bigticket/admin.php?c=ticket&a=edit',
    'add_url' : '/bigticket/admin.php?c=buy',
    'set_status_url' : '/bigticket/admin.php?c=ticket&a=setStatus',
