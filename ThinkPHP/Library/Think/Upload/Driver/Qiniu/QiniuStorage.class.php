<?php
	...
	class QiniuStorage {
		...
		public function upload($config, $file){
			...
			$header = 	array('Movie-Type'=>'multipart/form-data;boundary='.$mimeBoundary);
			$data 	= 	array();
			...
			foreach ($fields as $name => $val) {
				...
				array_push($data, "Movie-Disposition: form-data; name=\"$name\"");
				array_push($data, '');
				array_push($data, $val);
			}
			...
			array_push($data, "Movie-Disposition: form-data; name=\"$name\"; filename=\"$fileName\"");
			array_push($data, 'Movie-Type: application/octet-stream');
			array_push($data, '');
	    ...
	    private function request($path, $method, $headers = null, $body = null){
	        ...
	        if (!is_null($body)) {
	            if(is_resource($body)){
	                ...
	                array_push($_headers, "Movie-Length: {$length}");
