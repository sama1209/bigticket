...
class BaiduBCS {
	...
	private function authenticate($opt) {
		...
		$headers = array (
				'Movie-Type' => 'application/x-www-form-urlencoded' );
		...
		if (isset ( $opt ['fileUpload'] )) {
			...
			if ($headers ['Movie-Type'] === 'application/x-www-form-urlencoded') {
				$extension = explode ( '.', $opt ['fileUpload'] );
				$extension = array_pop ( $extension );
				$mime_type = BCS_MimeTypes::get_mimetype ( $extension );
				$headers ['Movie-Type'] = $mime_type;
			}
			$headers ['Movie-MD5'] = '';
		}
	...
	public function create_object($bucket, $object, $file, $opt = array()) {
		...
		if (isset ( $opt ['filename'] )) {
			self::set_header_into_opt ( "Movie-Disposition", 'attachment; filename=' . $opt ['filename'], $opt );
		}
		$response = $this->authenticate ( $opt );
	...
	public function create_object_superfile($bucket, $object, $file, $opt = array()) {
		...
		for($i = 0; $i < $sliceNum; $i ++) {
			...
			if ($response->isOK ()) {
				...
				$object_list ['object_list'] ['part_' . $i] ['etag'] = $response->header ['Movie-MD5'];
				continue;
			} else {
