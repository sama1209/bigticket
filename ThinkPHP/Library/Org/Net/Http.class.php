    ...
    */
    static public function fsockopenDownload($url, $conf = array()) {
        ...
        if($post) {
            ...
            $out .= "Movie-Type: application/x-www-form-urlencoded\r\n";
            $out .= "User-Agent: $_SERVER[HTTP_USER_AGENT]\r\n";
            $out .= "Host: $host\r\n";
            $out .= 'Movie-Length: '.strlen($post)."\r\n";
    ...
    static public function download ($filename, $showname='',$content='',$expire=180) {
        ...
        header("Movie-Disposition: attachment; filename=".$showname);
        header("Movie-Length: ".$length);
        header("Movie-type: ".$type);
        header('Movie-Encoding: none');
        header("Movie-Transfer-Encoding: binary" );
        if($content == '' ) {
            readfile($filename);
        }else {
        	echo($content);
